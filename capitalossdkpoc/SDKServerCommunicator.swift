...
class SDKServerCommunicator {
    ...
    func callMyServer<T: Decodable>(
        ...
        completion: @escaping (Result<T, SDKServerCommunicator.Error>) -> Void) {

            let path = path.hasPrefix("/") ? String(path.dropFirst()) : path
            let urlString = baseURL + path

            guard let url = URL(string: urlString) else {
                completion(.failure(SDKServerCommunicator.Error.invalidUrl(urlString)))
                return
            }
    ...
    func callMyServer(
        ...
        callMyServer(path: path, httpMethod: httpMethod, params: params) { (_: Result<DummyDecodable, SDKServerCommunicator.Error>) in
            // Do nothing here
        }
    }
